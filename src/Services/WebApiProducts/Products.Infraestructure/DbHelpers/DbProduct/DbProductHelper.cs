...
namespace Products.Infraestructure.DbHelpers
{
    ...
    {
        public Expression<Func<Product, bool>> FiltersExpression(string search, int productCategoryId, int providerId, decimal minPrice, decimal maxPrice, ProductUtilities.CONDITIONCODE condition)
        {
            Expression<Func<Product, bool>> filters =
                x => !x.Name.Equals("") 
                     && x.ProductCategoryId == productCategoryId 
                     && x.Status == true 
                     && x.ProductCondition.Code.Equals(condition.ToString());
